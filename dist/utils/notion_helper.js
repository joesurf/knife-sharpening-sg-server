...
export function getTextFromNotionProperty(property) {
...
export const getNotionPageIdByOrderNumber = async (orderNumber) => {
  try {
    const response = await notion.dataSources.query({
...
export const insertNotionOrder = async (order) => {
  try {
    const newOrderNumber = getNewOrderNumber(order.orderGroup, order.currentOrder);
...
export const getOrders = async ({ orderGroup, driverId, includeUrgent = false }) => {
  try {
    const filters = [
...
export const updateOrderConstantsToNextOrderGroup = async () => {
  ...
  const newPickupDate = format(addWeeks(parseISO(orderConstants.pickupDate), 1), 'yyyy-MM-dd');
  const newDeliveryDate = format(addWeeks(parseISO(orderConstants.deliveryDate), 1), 'yyyy-MM-dd');
  await notion.pages.update({
    page_id: ORDER_CONSTANTS_PAGE_ID,
    properties: {
      'Order Group': {
...
export function formatOrders(orders) {
  return orders
    .filter((order) => "properties" in order)
    .map(formatOrder);
}
