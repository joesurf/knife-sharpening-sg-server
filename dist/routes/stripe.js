...
import { stringifyAddressObject, getNewOrderNumber, formatDate, } from '../utils/utils.js';
import { insertNotionCustomer, insertNotionOrder, getOrderConstants, getNotionCustomerIdByPhone, updateNotionCustomerAddress, updateNotionCustomer180DayFollowUp, clearNotionCustomerReminderDate, } from '../utils/notion_helper.js';
...
router.post('/new-order', express.raw({ type: 'application/json' }), async (req, res) => {
    let event;
    ...
    switch (event.type) {
        case 'checkout.session.completed': {
            ...
            const additionalInstructions = eventData.custom_fields.find((field) => field.key === 'additional_instructions')?.text?.value || 'NA';
            ...
            const botspaceBody = {
                ...
                address: customerAddress,
                note: additionalInstructions,
                orderNumber: getNewOrderNumber(orderConstants.orderGroup, orderConstants.currentOrder),
